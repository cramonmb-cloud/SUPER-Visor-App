...
export const AdminPanel: React.FC<AdminPanelProps> = (props) => {
  ...
  const [showActiveCyclesModal, setShowActiveCyclesModal] = useState(false);
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'supervisors' && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div 
              onClick={() => setShowActiveCyclesModal(true)}
              className="bg-white p-4 rounded-xl shadow-sm border border-indigo-100 flex justify-between items-center bg-indigo-50/30 col-span-1 md:col-span-1 cursor-pointer hover:bg-indigo-50/80 hover:border-indigo-300 transition-all group"
              title="Haz clic para ver las semanas de cada financiera"
            >
                <div>
                    <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest flex items-center gap-1.5">
                      Ciclo Activo <Eye className="w-3 h-3 opacity-60 group-hover:opacity-100 transition-opacity" />
                    </p>
                    <h3 className="text-[11px] font-black text-indigo-900 uppercase group-hover:text-indigo-600 transition-colors">
                    ...
                    <p className="text-[9px] font-bold text-indigo-400 opacity-80 mt-0.5">Toca para ver desglose por financiera</p>
                </div>
                <div className="p-2.5 bg-indigo-100/50 rounded-lg group-hover:bg-indigo-600 group-hover:text-white transition-all text-indigo-600">
...
{`{
  ...
  "registeredAt": 171512345678...
      ...
      {/* MODAL DE CICLOS ACTIVOS POR FINANCIERA */}
      {showActiveCyclesModal && (
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 bg-gradient-to-r from-indigo-900 via-slate-900 to-indigo-950 text-white flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black uppercase tracking-tight">Ciclos por Financiera</h3>
                  <p className="text-[10px] font-bold text-indigo-200/80 uppercase tracking-widest">
                    Estado de semana actual por entidad
                  </p>
                </div>
              </div>
              <button 
                onClick={() => setShowActiveCyclesModal(false)}
            ...
            <div className="p-6 space-y-3 max-h-[65vh] overflow-y-auto">
              {data.financieras.map(fin => {
                const activeWeek = data.weeks
                  .filter(w => w.financieraId === fin.id && w.isActive)
                  .sort((a, b) => b.startDate - a.startDate)[0];

                const latestWeekAny = data.weeks
                  .filter(w => w.financieraId === fin.id)
                  .sort((a, b) => b.startDate - a.startDate)[0];

                const displayWeek = activeWeek || latestWeekAny;

                return (
                  <div key={fin.id} className="p-4 bg-slate-50 hover:bg-slate-100/80 rounded-2xl border border-slate-200/70 transition-all flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      {fin.logoUrl ? (
                        <div className="w-10 h-10 rounded-xl bg-white border border-slate-200 p-1 flex-shrink-0 shadow-sm flex items-center justify-center">
                          <img src={fin.logoUrl} alt={fin.name} className="max-h-full max-w-full object-contain" />
                        </div>
                      ) : (
                        <div className="w-10 h-10 rounded-xl bg-indigo-600 text-white font-black text-xs flex items-center justify-center uppercase shadow-sm flex-shrink-0">
                          {fin.name.substring(0, 3)}
                        </div>
                      )}
                      <div className="min-w-0">
                        <h4 className="text-xs font-black uppercase text-slate-800 truncate">{fin.name}</h4>
                        {displayWeek ? (
                          <div className="space-y-0.5 mt-0.5">
                            <p className="text-[11px] font-extrabold text-indigo-700 uppercase tracking-tight">
                              {displayWeek.name}
                            </p>
                            <p className="text-[9px] font-bold text-slate-400 uppercase">
                              {new Date(displayWeek.startDate).toLocaleDateString('es-MX', { day: '2-digit', month: 'short' })} - {new Date(displayWeek.endDate).toLocaleDateString('es-MX', { day: '2-digit', month: 'short', year: 'numeric' })}
                            </p>
                          </div>
                        ) : (
                          <p className="text-[10px] font-bold text-slate-400 italic">Sin semanas registradas</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-end flex-shrink-0">
                      {activeWeek ? (
                        <span className="px-3 py-1 bg-green-100 text-green-700 text-[9px] font-black uppercase rounded-full border border-green-200 tracking-wider flex items-center gap-1 shadow-xs">
                          <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span> ACTIVA
                        </span>
                      ) : (
                        <span className="px-3 py-1 bg-slate-200/70 text-slate-600 text-[9px] font-black uppercase rounded-full border border-slate-300 tracking-wider">
                          CERRADA
                        </span>
                      )}
                    </div>
              ...
              {data.financieras.length === 0 && (
                <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                  <p className="text-xs font-bold text-slate-400 uppercase">No hay financieras registradas</p>
                </div>
              )}
            </div>
